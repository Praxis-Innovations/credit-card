 ...
 * Resolves wallet from the request body; auth→user_cards is handled by the
 * route when ownedCardIds is omitted.
 */
export function buildRecommendationResponse(
  ...
  if (typeof amountCad !== "number" || !(amountCad > 0) || !Number.isFinite(amountCad)) {
    return {
      status: 400,
      error: {
        code: "bad_request",
